...
\vspace{3mm}
\begin{projectfiles}
\projectfile{d1\_01\_colab\_and\_coordinate\_click\_tool.ipynb}{使用固定圖片與 HTML canvas 滑鼠點選工具，取得相機畫面座標。}
\projectfile{d1\_02\_homography\_point\_projection.ipynb}{輸入相機點與 BEV 點，計算 Homography，並把指定點投影到 BEV。}
\projectfile{d1\_03\_keypoint\_annotation\_roboflow\_lab.ipynb}{介紹 keypoint 標註順序、誤差檢查與之後可延伸的 training code。}
\projectfile{d1\_04\_bbox\_homework\_setup.ipynb}{說明 player、ball、rim、number 等 bbox 類別與 Roboflow Dataset Version 作業。}
\end{projectfiles}
...
\vspace{3mm}
\begin{projectfiles}
\projectfile{d2\_01\_manual\_detection\_box\_to\_bev.ipynb}{先用單一寫死 bbox 了解 bottom-center 與 BEV 投影。}
\projectfile{d2\_02\_yolo26\_detection\_preview.ipynb}{若有權重就跑 YOLO26 / Ultralytics detection；否則使用 sample detections JSON。}
\projectfile{d2\_03\_roboflow\_bbox\_training\_preview.ipynb}{保留 Roboflow dataset download 與 training code 骨架，課堂以 preview 為主。}
\projectfile{d2\_04\_bbox\_to\_bev\_integration.ipynb}{將多個 detection boxes 的 footpoints 投影到 BEV。}
\end{projectfiles}
...
\vspace{3mm}
\begin{projectfiles}
\projectfile{d3\_01\_tracking\_concept\_iou\_association.ipynb}{用 IoU matrix 了解 detection boxes 如何跨 frame 配對。}
\projectfile{d3\_02\_bytetrack\_demo\_with\_sample\_boxes.ipynb}{用 sample boxes 體驗 Track ID 與追蹤結果圖。}
\projectfile{d3\_03\_tracking\_to\_bev\_mini\_project.ipynb}{將 Track ID 路徑投影到 BEV，完成前三天整合。}
\end{projectfiles}
...
\vspace{3mm}
\begin{projectfiles}
\projectfile{d4\_01\_upload\_and\_convert\_shooting\_video.ipynb}{整理學生影片上傳與 ffmpeg 統一轉檔流程。}
\projectfile{d4\_02\_mediapipe\_pose\_angle\_lab.ipynb}{使用 MediaPipe 或合成資料計算投籃姿態角度。}
\projectfile{d4\_03\_ball\_tracking\_and\_release\_point\_lab.ipynb}{用簡單球追蹤取得 ball path 與 release frame 觀察。}
\end{projectfiles}
...
\vspace{3mm}
\begin{projectfiles}
\projectfile{d5\_01\_shooting\_analysis\_pipeline\_runner.ipynb}{讀取 Day 4 結果並產生投籃分析 summary。}
\projectfile{d5\_02\_report\_visualization\_builder.ipynb}{整理角度圖、球軌跡圖與骨架展示圖。}
\projectfile{d5\_03\_showcase\_export.ipynb}{將成果圖與 summary 匯出成展示用 zip。}
\end{projectfiles}
