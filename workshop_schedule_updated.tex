% !TeX program = xelatex
% chktex-file 1
% chktex-file 44
% XeLaTeX-only version for Source Han Sans / Noto Sans CJK typography.
\documentclass[11pt]{article}
\usepackage[margin=0.85in]{geometry}

\usepackage{iftex}
\ifXeTeX
\else
\PackageError{main}{This document requires XeLaTeX}{Please compile with xelatex.}
\fi
\usepackage{fontspec}
\usepackage{xeCJK}

\setmainfont{TeX Gyre Heros}
\setsansfont{TeX Gyre Heros}
\setmonofont{TeX Gyre Cursor}
\setCJKmainfont{Noto Sans TC}[UprightFont = *, BoldFont = * Bold, AutoFakeSlant = 0.15]
\setCJKsansfont{Noto Sans TC}[UprightFont = *, BoldFont = * Bold, AutoFakeSlant = 0.15]
\setCJKmonofont{Noto Sans TC}[UprightFont = *, BoldFont = * Bold, AutoFakeSlant = 0.15]
\renewcommand{\CJKfamilydefault}{\CJKsfdefault}

\usepackage{amsmath,amssymb}
\usepackage{tikz-cd}
\usepackage{multicol}
\usepackage[table]{xcolor}
\usepackage{tabularx}
\usepackage{array}
\usepackage{enumitem}
\usepackage{titlesec}
\usepackage{tcolorbox}
\usepackage[bookmarks=false,hidelinks]{hyperref}
\renewcommand{\familydefault}{\sfdefault}

\setlength{\parindent}{0pt}
\setlength{\parskip}{0.45\baselineskip}
\renewcommand{\arraystretch}{1.18}
\setlength{\tabcolsep}{4.8pt}
\emergencystretch=2em
\sloppy

\definecolor{MainBlue}{HTML}{3A3A3A}
\definecolor{DeepNavy}{HTML}{111111}
\definecolor{AccentTeal}{HTML}{555555}
\definecolor{AccentCoral}{HTML}{333333}
\definecolor{LightBlue}{HTML}{F5F5F5}
\definecolor{SoftGray}{HTML}{FAFAFA}
\definecolor{DarkGray}{HTML}{333333}
\definecolor{WarnYellow}{HTML}{EDEDED}

\newcommand{\overviewitem}[4]{%
  \noindent
  \begin{minipage}[t]{0.12\textwidth}
    \textbf{#1}\\[-0.5mm]
    {\scriptsize #4}
  \end{minipage}
  \hfill
  \begin{minipage}[t]{0.84\textwidth}
    {\scriptsize\textbf{講義課：} #2}\\[-0.2mm]
    {\scriptsize\textbf{實作：} #3}
  \end{minipage}
  \par\vspace{1.2mm}
  \hrule
  \vspace{1.2mm}
}

\tcbuselibrary{breakable}
\newenvironment{projectfiles}{
  \vspace{2.5mm}
  \begin{tcolorbox}[colback=SoftGray,colframe=DarkGray,arc=1.5mm,boxrule=0.45pt,left=2.5mm,right=2.5mm,top=2mm,bottom=1.5mm,breakable]
    {\normalsize\bfseries 當日工作坊專案檔案}\\[-0.2mm]
    \begin{enumerate}[leftmargin=1.7em,itemsep=2pt,topsep=2pt,parsep=0pt]
      }{
    \end{enumerate}
  \end{tcolorbox}
}

\titleformat{\section}{\Large\bfseries\color{MainBlue}}{}{0pt}{}
\titleformat{\subsection}{\large\bfseries\color{DarkGray}}{}{0pt}{}
\renewcommand{\tabularxcolumn}[1]{m{#1}}
\newcolumntype{Y}{>{\raggedright\arraybackslash}X}
\newcolumntype{C}[1]{>{\centering\arraybackslash}m{#1}}
\newcolumntype{L}[1]{>{\raggedright\arraybackslash}m{#1}}
\newcommand{\checkbox}{\(\square\)}
\newcommand{\donebox}{\(\blacksquare\)}
\newcommand{\typebadge}[1]{\colorbox{LightBlue}{\textcolor{MainBlue}{\scriptsize\bfseries\strut\ #1\ }}}
\newcommand{\checks}[1]{\begin{itemize}[leftmargin=1.45em,itemsep=0pt,topsep=0pt,parsep=0pt,label=\(\square\)]#1\end{itemize}}
\newcommand{\daybox}[2]{%
  \begin{tcolorbox}[colback=SoftGray,colframe=DarkGray,arc=1mm,boxrule=0.45pt,left=2.5mm,right=2.5mm,top=2mm,bottom=2mm]
    \textbf{核心目標：} #1

    \vspace{1mm}
    \textbf{當日產出：} #2
  \end{tcolorbox}
}
\newcommand{\projectfile}[2]{\item {\ttfamily\small #1}\\[-0.3mm]{\footnotesize #2}}

\begin{document}
\newcommand{\tasktableheader}{%
  \rowcolor{MainBlue}
  \textcolor{white}{\textbf{完成}} &
  \textcolor{white}{\textbf{編號}} &
  \textcolor{white}{\textbf{類型}} &
  \textcolor{white}{\textbf{任務內容}} &
  \textcolor{white}{\textbf{審查方式}} &
  \textcolor{white}{\textbf{分數}} \\ \hline
}

\begin{center}
  {\Huge \textbf{Python「黑客松」實戰分析籃球運動數據}}\\[4pt]
  {\Large 五天課程時間表與任務審查表}\\[10pt]
  \rule{0.9\textwidth}{0.6pt}\\[8pt]
  {\large \textcolor{DarkGray}{Colab｜Roboflow｜Homography｜Detection｜ByteTrack｜Pose｜Shooting Analysis}}
\end{center}
\vspace{2mm}

\begin{tcolorbox}[colback=SoftGray,colframe=DarkGray,arc=1mm,boxrule=0.45pt,left=3mm,right=3mm,top=2mm,bottom=2mm]
  {\large\bfseries 課程總覽}
  \vspace{2mm}\hrule\vspace{1.5mm}

\overviewitem{Day 1}{Colab、影像座標、keypoint 配對、Homography 與 BEV projection。}{固定圖片與 BEV 圖 keypoint 配對、單點投影、Roboflow keypoint 與 bbox 作業準備。}{20 分}
  \overviewitem{Day 2}{Object Detection、Bounding Box、confidence、bottom-center footpoint 與 BBOX-to-BEV。}{先用寫死框體驗，再用 sample / trained YOLO26 detection，最後把多個 player box 投影到 BEV。}{25 分}
  \overviewitem{Day 3}{多目標追蹤、IoU 關聯、ByteTrack、Track ID、BEV 軌跡與隊伍分群。}{將 detections 接成 tracks，投影到 BEV，再用球衣 torso crop 與 HSV 色彩特徵分成 Team A / B。}{20 分}
  \overviewitem{Day 4}{近距離投籃視角、課程提供的 shot／pose／ball 模型、投籃事件與出手角度。}{從 Google Drive 或 Colab 上傳自己的投籃影片，直接推論投籃者、人體骨架與球軌跡，再估計 release frame、關節角度與 2D 起飛角。}{15 分}
  \overviewitem{Day 5}{研究問題、系統架構、評估設計、限制與技術溝通。}{各組提出 Project Proposal，或展示已完成的籃球視覺分析專案 Demo。}{20 分}

  \noindent
  \begin{minipage}[t]{0.12\textwidth}\textbf{加分}\\[-0.5mm]{\scriptsize +10}\end{minipage}
  \hfill
  \begin{minipage}[t]{0.84\textwidth}{\scriptsize 改善 demo、補充失敗案例分析、調整視覺化、完成額外影片分析，或提交具體課程回饋。}\end{minipage}

  \vspace{2mm}\hrule\vspace{1.5mm}
  \hfill {\bfseries 總分：100 (+10)}
\end{tcolorbox}

\clearpage

\section{Day 1｜Colab、Keypoint 配對、Homography 與資料標註準備}
\daybox{讓學生熟悉 Colab、課程資料夾與影像座標。課程以固定圖片與 BEV 圖建立 keypoint 對應關係，再用 Homography 將相機座標轉換到鳥瞰座標。最後介紹 Roboflow keypoint 與 bounding box 標註方式，讓學生準備 Day 2 會用到的少量資料。}{Colab 可正常執行、相機畫面座標點、BEV 對應點、Homography matrix、單點投影結果、keypoint / bbox 標註作業格式。}

\vspace{1mm}
\footnotesize
\begin{tabularx}{\textwidth}{|C{0.85cm}|C{1.2cm}|C{1.55cm}|Y|Y|C{0.75cm}|}\hline
\tasktableheader
\checkbox & 1.1 & \typebadge{環境} & 開啟課程 repo，確認 Colab、Google Drive、assets 與 src 路徑可正常讀取。 & \checks{\item Notebook 可成功顯示 COURSE\_ROOT 與 sample image} & 3 \\ \hline
\checkbox & 1.2 & \typebadge{座標} & 使用 BEV / camera keypoint 配對工具建立對應點。 & \checks{\item 至少完成 4 組 keypoint pair} & 4 \\ \hline
\checkbox & 1.3 & \typebadge{BEV} & 檢查每一組 pair 的 BEV keypoint 名稱與相機畫面位置。 & \checks{\item 相機點與 BEV 點數量一致，且 pair 名稱可解釋} & 3 \\ \hline
\checkbox & 1.4 & \typebadge{轉換} & 使用 cv2.findHomography 計算矩陣，並指定一個點投影到 BEV。 & \checks{\item 可輸出 Homography matrix 與單點投影結果圖} & 5 \\ \hline
\checkbox & 1.5 & \typebadge{標註} & 介紹 Roboflow keypoint 與 bbox 作業，學生練習標少量圖片。 & \checks{\item keypoint / bbox 作業格式確認完成} & 5 \\ \hline
\rowcolor{LightBlue}\multicolumn{5}{|r|}{\textbf{Day 1 小計}} & \textbf{20} \\ \hline
\end{tabularx}
\normalsize
\vspace{3mm}
\begin{projectfiles}
\projectfile{day1/d1\_01\_keypoint\_pairing\_homography.ipynb}{使用 BEV / camera keypoint 配對工具建立對應點，計算 Homography，並把指定點投影到 BEV。}
\projectfile{day1/d1\_02\_keypoint\_annotation\_roboflow\_lab.ipynb}{介紹 keypoint 標註順序、資料格式、誤差檢查與可延伸的 fine-tuning 流程。}
\projectfile{day1/d1\_03\_bbox\_homework.ipynb}{準備 Day 2 使用的 player、ball、rim、number 等 bbox 資料與 Roboflow Dataset Version。}
\end{projectfiles}

\clearpage
\section{Day 2｜Detection Preview、YOLO26 與 BBOX-to-BEV}
\daybox{讓學生理解 object detection 的輸出就是一組組 bounding boxes，而不是一開始就要求他們訓練完整模型。課程先用寫死的 player box 觀察 bottom-center footpoint，再用 sample detections 或老師提供的 trained YOLO26 model 看 detection 結果，最後把多個 player footpoints 投影到 BEV。}{單一 bbox-to-BEV 結果、detection preview 圖、training preview 概念、多人 BBOX-to-BEV 投影結果。}

\vspace{1mm}\footnotesize
\begin{tabularx}{\textwidth}{|C{0.85cm}|C{1.2cm}|C{1.55cm}|Y|Y|C{0.75cm}|}\hline
\tasktableheader
\checkbox & 2.1 & \typebadge{框體} & 使用寫死的一個 player bbox，取 bottom-center 作為球員站在地板上的位置。 & \checks{\item 可畫出 player box 與 bottom-center 點} & 4 \\ \hline
\checkbox & 2.2 & \typebadge{投影} & 沿用 Day 1 Homography，將單一 player footpoint 投影到 BEV。 & \checks{\item 可輸出左圖 bbox、右圖 BEV 點的 side-by-side 圖} & 5 \\ \hline
\checkbox & 2.3 & \typebadge{偵測} & 使用 sample detections 或 trained YOLO26 / Ultralytics-style model 觀察 player、ball、rim、number 偵測結果。 & \checks{\item Notebook 可輸出 detection preview 圖} & 6 \\ \hline
\checkbox & 2.4 & \typebadge{資料} & 檢查 Roboflow bbox dataset 與 training preview，理解 dataset、class、confidence 與模型限制。 & \checks{\item 能說明一個 detection 可能失敗的原因} & 4 \\ \hline
\checkbox & 2.5 & \typebadge{整合} & 將多個 player bounding boxes 轉成 footpoints 並投影到 BEV。 & \checks{\item 可輸出多人 BBOX-to-BEV 結果圖} & 6 \\ \hline
\rowcolor{LightBlue}\multicolumn{5}{|r|}{\textbf{Day 2 小計}} & \textbf{25} \\ \hline
\end{tabularx}
\normalsize
\vspace{3mm}
\begin{projectfiles}
\projectfile{day2/d2\_01\_yolo26\_detection.ipynb}{承接 Day 1 的資料準備，專注解讀 class、confidence、bbox 與短影片偵測結果。}
\projectfile{day2/d2\_02\_yolo\_players\_to\_bev.ipynb}{將多個 player boxes 的 bottom-center footpoints 投影到 BEV。}
\projectfile{day2/d2\_03\_bbox\_to\_bev\_integration.ipynb}{逐 frame 整合 detection、court keypoints、Homography 與 BEV 輸出。}
\end{projectfiles}

\clearpage
\section{Day 3｜ByteTrack、多目標追蹤、BEV 路徑與隊伍分群}
\daybox{完成 Day 1 到 Day 3 的位置資料篇章：從影像座標、Homography、Detection 到 Tracking。學生先用 IoU 理解跨 frame box 關聯，再觀察 ByteTrack 的 Track ID 與軌跡，把每個 ID 投影到 BEV，最後用球衣 torso crop、HSV histogram 與 K-means 分成兩隊。}{IoU heatmap 與關聯線、含軌跡的 Track ID 影片、BEV 路徑資料、Team A / B 分群圖。}

\vspace{1mm}\footnotesize
\begin{tabularx}{\textwidth}{|C{0.85cm}|C{1.2cm}|C{1.55cm}|Y|Y|C{0.75cm}|}\hline
\tasktableheader
\checkbox & 3.1 & \typebadge{概念} & 使用連續兩個 frames 的 boxes 計算 IoU，理解 box association。 & \checks{\item 可輸出 IoU matrix 並說明如何配對} & 4 \\ \hline
\checkbox & 3.2 & \typebadge{追蹤} & 使用 ByteTrack 概念或簡化 tracker，將多個 frames 的 detections 賦予 Track ID。 & \checks{\item 可輸出含 Track ID 的 frame 圖} & 5 \\ \hline
\checkbox & 3.3 & \typebadge{軌跡} & 整理每個 Track ID 的 center / footpoint 座標，形成球員路徑資料。 & \checks{\item 可輸出 tracks json 或 dataframe} & 4 \\ \hline
\checkbox & 3.4 & \typebadge{BEV} & 將 track footpoints 投影到 BEV，畫出每個 ID 的移動路徑。 & \checks{\item 可輸出 BEV 軌跡圖} & 5 \\ \hline
\checkbox & 3.5 & \typebadge{分群} & 裁切球員上半身，使用 HSV 色彩特徵與 K-means 分成 Team A / B，並討論 Track ID 與真實球員身分的差異。 & \checks{\item 可對照 BBOX、torso crops、2D 特徵圖與隊伍結果；能指出一個分群限制} & 2 \\ \hline
\rowcolor{LightBlue}\multicolumn{5}{|r|}{\textbf{Day 3 小計}} & \textbf{20} \\ \hline
\end{tabularx}
\normalsize
\vspace{3mm}
\begin{projectfiles}
\projectfile{day3/d3\_01\_tracking\_concept\_iou\_association.ipynb}{用 IoU matrix 了解 detection boxes 如何跨 frame 配對。}
\projectfile{day3/d3\_02\_yolo\_bytetrack\_tracking.ipynb}{使用 YOLO + ByteTrack 觀察 Track ID、短軌跡與 track lifecycle 摘要。}
\projectfile{day3/d3\_03\_team\_clustering.ipynb}{使用 torso crop、HSV histogram 與兩群 K-means 建立可視化隊伍分群。}
\projectfile{day3/d3\_04\_tracking\_to\_bev\_mini\_project.ipynb}{整合 Track ID、BEV 路徑與 track-level 隊伍分群，輸出原圖隊伍框、特徵散點與隊伍色 BEV 路徑。}
\end{projectfiles}

\clearpage
\section{Day 4｜近距離投籃影片、人體姿態與球軌跡分析}
\daybox{課程第二個篇章切換到近距離投籃視角。學生用手機拍攝投籃影片，放入 Google Drive 的 assets/raw，或從 Day 4-01 的 Colab 上傳格直接選檔，再統一轉成 mp4。接著使用課程提供的 shot detector、YOLO pose 與 ball detector，融合投籃候選分數、手腕位置和球軌跡，估計 release frame、關節角度與 2D 起飛角。}{轉檔後投籃影片、shot event CSV、pose angle CSV、事件球軌跡、角度分析圖、release 檢查圖與 overlay 影片。}

\vspace{1mm}\footnotesize
\begin{tabularx}{\textwidth}{|C{0.85cm}|C{1.2cm}|C{1.55cm}|Y|Y|C{0.75cm}|}\hline
\tasktableheader
\checkbox & 4.1 & \typebadge{上傳} & 將投籃影片放入 Google Drive 的 assets/raw，或在 Day 4-01 設定 USE\_BROWSER\_UPLOAD=True 後直接選檔。 & \checks{\item using video 顯示預期的 converted 影片} & 3 \\ \hline
\checkbox & 4.2 & \typebadge{轉檔} & 使用 ffmpeg 將影片統一轉成 mp4、30 FPS、最大邊 1280、移除音訊。 & \checks{\item assets/converted 中出現 mp4} & 3 \\ \hline
\checkbox & 4.3 & \typebadge{事件} & 使用 shot detector 與 YOLO pose 鎖定投籃者和投籃候選時間區段。 & \checks{\item 可輸出逐 frame shot score 與 pose dataframe} & 3 \\ \hline
\checkbox & 4.4 & \typebadge{角度} & 計算 elbow、knee、shoulder angle，並在 release frame 讀取投籃側關節角度。 & \checks{\item 可輸出角度變化圖與 release 關節角度} & 3 \\ \hline
\checkbox & 4.5 & \typebadge{球跡} & 從投籃者手腕附近建立事件專屬球軌跡，估計 release frame 與 2D 起飛角。 & \checks{\item 可輸出事件 CSV、起飛角與人工檢查圖} & 3 \\ \hline
\rowcolor{LightBlue}\multicolumn{5}{|r|}{\textbf{Day 4 小計}} & \textbf{15} \\ \hline
\end{tabularx}
\normalsize
\vspace{3mm}
\begin{projectfiles}
\projectfile{day4/d4\_01\_ball\_detector\_bytetrack\_preview.ipynb}{準備自己的影片，並使用課程提供的 ball detector 與 ByteTrack 檢查球框和 track ID。}
\projectfile{day4/d4\_02\_yolo\_pose\_and\_shot\_lab.ipynb}{使用 shot detector 與 YOLO pose 取得投籃候選分數和投籃者姿態角度。}
\projectfile{day4/d4\_03\_ball\_tracking\_and\_release\_point\_lab.ipynb}{融合事件、手腕與球軌跡，輸出 release frame、關節角度、2D 起飛角與視覺檢查。}
\end{projectfiles}

\clearpage
\section{Day 5｜Project Proposal 與完成專案 Demo 報告}
\daybox{Day 5 不再加入固定的新實作流程。各組使用前四天工作坊所學，提出一個可執行、可評估的籃球視覺分析 Project Proposal，或展示已完成的專案 Demo。報告需從問題、資料、方法一路說到證據、限制與下一步。}{6--10 頁簡報、一頁摘要、系統架構圖、評估設計，以及 Proposal 的里程碑／預期結果或 Completed Demo 的可重現輸出。}

\vspace{1mm}\footnotesize
\begin{tabularx}{\textwidth}{|C{0.85cm}|C{1.2cm}|C{1.55cm}|Y|Y|C{0.75cm}|}\hline
\tasktableheader
\checkbox & 5.1 & \typebadge{問題} & 定義目標使用者、輸入、輸出與一到三個可回答的研究／產品問題。 & \checks{\item 問題明確，不只羅列模型名稱} & 4 \\ \hline
\checkbox & 5.2 & \typebadge{資料} & 說明資料來源、標註、切分、授權與可假設的先決模組。 & \checks{\item 已完成、沿用與規劃中內容有清楚區分} & 3 \\ \hline
\checkbox & 5.3 & \typebadge{方法} & 以架構圖呈現 video、detections、tracks、team / identity、BEV / pose / events 與最終輸出的資料流。 & \checks{\item 每個模組的輸入、輸出與主要假設可解釋} & 4 \\ \hline
\checkbox & 5.4 & \typebadge{證據} & Proposal 提出 baseline、metrics 與預期圖表；Completed Demo 提供代表性成功與失敗案例。 & \checks{\item 評估方法能回答原始問題} & 4 \\ \hline
\checkbox & 5.5 & \typebadge{展示} & 依時限完成 Proposal 或 Demo 報告，使用圖表、影片或 storyboard 支持結論。 & \checks{\item 簡報、摘要與必要附件完成} & 3 \\ \hline
\checkbox & 5.6 & \typebadge{反思} & 說明限制、風險、錯誤傳遞與有優先順序的下一步。 & \checks{\item 至少提出一項可執行的後續實驗} & 2 \\ \hline
\rowcolor{LightBlue}\multicolumn{5}{|r|}{\textbf{Day 5 小計}} & \textbf{20} \\ \hline
\end{tabularx}
\normalsize
\vspace{3mm}
\begin{projectfiles}
\projectfile{day5/project\_proposal\_spec.tex}{Proposal / Completed Project Demo 的交付格式、報告節奏、評分規準與題目方向。}
\projectfile{daily\_lecture\_outline.tex}{老師每日前一小時的高層次講義主軸；Day 5 包含研究設計、評估與技術溝通。}
\end{projectfiles}

\clearpage
\section{額外加分項目}
\footnotesize
\begin{tabularx}{\textwidth}{|C{0.85cm}|C{1.2cm}|C{1.55cm}|Y|Y|C{0.75cm}|}\hline
\tasktableheader
\checkbox & E.1 & \typebadge{加分} & 嘗試改善任一 demo，例如手動修正 Homography 點、調整 ball threshold、補上自己的影片分析，或改善成果圖。 & \checks{\item 有額外修改，並能展示修改後結果} & +5 \\ \hline
\checkbox & E.2 & \typebadge{回饋} & 提供課程回饋與改進建議，包含工具使用、任務難度、教材內容或未來想延伸的方向。 & \checks{\item 有提交具體回饋或建議} & +5 \\ \hline
\rowcolor{WarnYellow}\multicolumn{5}{|r|}{\textbf{額外加分}} & \textbf{+10} \\ \hline
\end{tabularx}
\normalsize

\end{document}
